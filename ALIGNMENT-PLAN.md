...
| 11 | zsh | `languages/zsh/valid/palette.zsh` | **done (no changes)** | ~50 → 62 (precise count first measured 2026-07-11 during the json pass's regression sweep, identical with and without the json changes — the original "~50" was a loose estimate, not a regression) | see note below the table — most mismatches are a bash-vs-zsh vim dialect conflict, not fixable without regressing the completed bash work |
...
| 13 | powershell | `languages/powershell/valid/palette.ps1` | **done** (revisited 2026-07-11) | 78 → 32 → 27 | `.SYNOPSIS`/`.DESCRIPTION` help tags — resolved (32→32, landed on master before this revisit): the grammar splits the token into a base `comment.documentation.embedded.powershell` scope plus nested captures (`constant.string.documentation.powershell` for the dot, `keyword.operator.documentation.powershell` for the keyword); vim's `ps1CommentDoc` colors the whole `.SYNOPSIS` token as one unit (`Tag`), so all three scopes now share that color. **This revisit found 3 of the remaining groups were genuinely fixable, not gaps as previously recorded (32→27):** `$_` was targeting the wrong scope entirely — the theme's `variable.language.powershell` rule is dead code for `$_` (that scope is real, but only matches preference variables like `$ErrorActionPreference`; `$_`'s actual grammar scope is `support.variable.automatic.powershell`, confirmed by reading the grammar's `variable` repository patterns in match order), so it fell through to the generic yellow `support` rule meant for cmdlet names — retargeted to the real scope; `[int[]]`/`[double]`/`[PSCustomObject]`-style type-annotation brackets get a dedicated rule for their otherwise-bare `punctuation.section.bracket.*.powershell`, disambiguated from the structurally-identical scope on attribute brackets (`[CmdletBinding()]`, `[Parameter()]`) via the `meta.attribute.powershell` ancestor the grammar already wraps those in (confirmed via vim's raw HTML dump that `[int[]]` is one uniform Type span while `[CmdletBinding()]` gets no span at all, and via raw `bat --color=always \| cat -v` that `[string]`/`[double]` were only "accidentally" correct before this fix, via the compare script's run-majority letter-count, not per-character); `function`/`filter`/`configuration`/`workflow` shares `storage.type.powershell` with type annotations but the grammar wraps the whole keyword+name match in its own `meta.function.powershell` ancestor, used to retarget just the keyword to Keyword-red. **Confirmed still-unfixable (27 remaining):** hashtable keys share `variable.other.readwrite.powershell` with real `$variable` references, no distinguishing scope at all — majority kept, reconfirmed by direct count (34 real variable-reference tokens vs 18 hashtable keys in this sample) (18); cmdlet parameter flags like `-Version`/`-Rgb`/`-ExpandProperty` have zero scope on the flag name — the leading `-` is incidentally caught by the generic arithmetic-operator pattern (already correctly colored) but the word following it is completely unscoped, confirmed via raw bat render matching plain body text exactly and via full top-level grammar pattern read (no pattern matches a bare post-dash identifier outside of the hardcoded comparison/logical/bitwise operator keyword lists) (6); `ForEach-Object`/`Where-Object` have their own dedicated grammar match rules but are assigned the identical `support.function.powershell` scope as ordinary Verb-Noun cmdlets, nothing to select on (2); `foreach` shares the catch-all `keyword.control.powershell` scope with if/switch/else/default/for/while/etc, with no wrapping meta scope to disambiguate on (1) |
...
| 17 | json | `languages/json/valid/palette.json` | **done** (2026-07-11) | 166 → 1 | `null` shares the grammar's single `constant.language.json` match (`\b(?:true\|false\|null)\b`) with `true`/`false`, which vim splits into jsonNull→Function-yellow vs jsonBoolean→Boolean-pink; booleans already render correctly and recoloring the shared scope would break both, so the lone `null` stays pink. Fixes this pass (all stock json.vim intents — srcery-vim itself only links `jsonKeyword→Identifier`, already handled by the existing key rule): quote marks `punctuation.definition.string.{begin,end}.json` → Normal `#FCE8C3` (vim Quote is unstyled, 130 tokens); braces/brackets `punctuation.section.{mapping,sequence}.{begin,end}.json` → Delimiter `#917E6B` (34); `constant.character.escape.json` → Special `#2C78BF` (1) |
