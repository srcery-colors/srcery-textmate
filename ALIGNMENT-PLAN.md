...
| 13 | powershell | `languages/powershell/valid/palette.ps1` | **done** | 78 → 34 | hashtable keys share a scope with `$variable` refs (majority kept); `function` shares a scope with type annotations (majority kept); ForEach-Object/Where-Object are hardcoded into the same match as ordinary cmdlets; `foreach` shares the control-keyword scope with if/switch/else/default; `$_`, `[int[]]`, `-Flag` parameters, and `.SYNOPSIS`/`.DESCRIPTION` help tags have unidentified scopes — probed but not found in time, open question for a future pass |
